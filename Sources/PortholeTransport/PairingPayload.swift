...
/// `porthole://pair?host=<addr>&port=<n>&pin=<hex>&name=<label>`.
///
...
public struct PairingPayload: Equatable, Sendable {
    ...
    /// The `porthole://pair?…` URL string to encode in a QR code.
    public var urlString: String {
        var components = URLComponents()
        components.scheme = "porthole"
    ...
    /// Parse a scanned `porthole://pair?…` URL; nil if malformed.
