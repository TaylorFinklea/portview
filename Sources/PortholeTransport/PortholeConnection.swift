...
/// One Porthole session over a secure connection.
///
...
public final class PortholeConnection: @unchecked Sendable {
    ...
    public static func connect(to endpoint: NWEndpoint, pinnedCertificateSHA256: Data) async throws -> PortholeConnection {
        let queue = DispatchQueue(label: "porthole.connection")
        let nw = NWConnection(to: endpoint, using: TLSParameters.client(pinnedCertificateSHA256: pinnedCertificateSHA256))
        let connection = PortholeConnection(connection: nw, queue: queue)
...
/// Accepts incoming Porthole QUIC connections (host side).
public final class PortholeListener: @unchecked Sendable {
    private let listener: NWListener
    private let queue: DispatchQueue
    private let connectionContinuation: AsyncStream<PortholeConnection>.Continuation
