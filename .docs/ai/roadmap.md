...
- [ ] **Magnifier on-device follow-up** (after the above is verified): if the crop output-resize causes stutter/keyframe flashes on zoom steps, consider a small set of discrete output sizes (snap zoom to levels) or interpolating between host confirmations.
