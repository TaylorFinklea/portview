...
### Now
- [ ] **Magnifier crisp-region streaming (high-zoom usability — THE blocker)** — on-device 2026-06-15: at the zoom needed to read text on a phone (~5×) the image is blurry/distorted, unusable vs VNC. **Root cause diagnosed** (3440×1440 ultrawide + portrait phone): `ZoomGeometry.cropRequest` builds a SQUARE box `side = max(visW, visH)`; for an ultrawide on a portrait phone the visible window stays ~full-height (`windowH = (1/ratio)/z ≈ 1.0` until zoom > displayAspect/viewAspect ≈ 5.2), so `side = max(0.2, 1.0) = 1.0` → **the host never crops** → the client digitally zooms into the full 3440×1440 frame encoded at ~1.3 Mbps → blur. Real fix = region streaming like VNC: crop host `sourceRect` to the actual visible window (NOT a square) **and resize the encoder OUTPUT dims to the crop's pixel aspect** so it isn't stretched. ⚠️ landmine: the prior tight-crop/native-res attempt (decisions 2026-06-04) **crashed + stretched** precisely because output stayed display-sized — must reconfigure output dims and device-verify each step. `tier_floor: lead`, `complexity: L`.
