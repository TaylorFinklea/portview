...
- [ ] **Magnifier on-device follow-up** (after the ladder is verified): if zoom feels notchy at rung crossings, densify the ladder (ratio 0.8→0.9) or interpolate `frameViewport` between host confirmations; if crispness steps are visible, raise encode bitrate when cropped. Also (qwen review 2026-06-16, MEDIUM, pre-existing): the `.viewport` echo and video frames are sent from concurrent tasks, so at a rung crossing a new-dimension frame can arrive ~1 frame (~16ms) before the echo → transient misalignment. Clean fix = carry the normalized viewport rect IN the `VideoFrame` message (it already carries width/height) so the client always has the matching region; eliminates the race. `tier_floor: senior`, `complexity: S`.
- [ ] **Motion choppiness on pan/move** — on-device 2026-06-15: moving the cursor/viewport is choppy. (Partially addressed 2026-06-16 by M7-P3 `OutboundInputPump` pointer-move coalescing — no longer replays a backlog of stale moves after a send stall; device-verify whether choppiness remains, then pursue the suspects below.) Suspects: the 250 ms viewport-request debounce + `SCStream.updateConfiguration` stutter on the interactive path, and/or the cursor-follow spring fighting throttled host `CursorPosition` reports. Try: keep visual pan fully decoupled from host re-crop, raise the `CursorPosition` report rate, interpolate `frameViewport` between host confirmations. `tier_floor: senior`, `complexity: M`.
...
- [ ] Tight A/V lip-sync.
- [ ] **6-digit one-time pairing code for Bonjour-discovered Macs** (user request 2026-06-15) — **DESIGN SPEC'D 2026-06-16, awaiting human security review** before implementation: `docs/superpowers/specs/2026-06-15-sas-pairing-design.md` (a cert-comparison SAS scheme; must-review items enumerated). typing the 64-hex pin is painful; QR stays the preferred path, but when a Mac is discovered via Bonjour let the user enter a short **6-digit OTP** the host displays instead of the full pin. Needs a host-side OTP→identity step over the pairing channel (the OTP authorizes obtaining/deriving the pinned cert), keeping cert-pinning as the trust anchor — so the OTP gates discovery convenience without weakening pinning. `tier_floor: lead` (security-sensitive), `complexity: M`.
