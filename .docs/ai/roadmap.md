...
- [ ] Tight A/V lip-sync.
- [ ] **6-digit one-time pairing code for Bonjour-discovered Macs** (user request 2026-06-15) — typing the 64-hex pin is painful; QR stays the preferred path, but when a Mac is discovered via Bonjour let the user enter a short **6-digit OTP** the host displays instead of the full pin. Needs a host-side OTP→identity step over the pairing channel (the OTP authorizes obtaining/deriving the pinned cert), keeping cert-pinning as the trust anchor — so the OTP gates discovery convenience without weakening pinning. `tier_floor: lead` (security-sensitive), `complexity: M`.
- [ ] **Menu-bar host (`MenuBarExtra`)** (user idea 2026-06-15) — make the Mac host live in the menu bar: click the menu-bar item to pop a compact panel showing the pairing QR for a quick connect, with a button to mint/show the 6-digit OTP (pairs with the item above). Lighter-weight than a full window for the common "just let me connect" flow; the full window can remain available. SwiftUI `MenuBarExtra` scene (per swiftui-expert macOS guidance), reusing the existing `HostAppModel`/QR. `tier_floor: senior`, `complexity: M`.
