...
- **Zoom — blurry = FIXED via host-side magnifier** (commit `235bb88`, user chose "build full magnifier now"). New `Viewport` msg (18, normalized, both directions): client sends target crop (1/zoom, cursor-centered, throttled ~12 Hz); host sets `SCStreamConfiguration.sourceRect` to it (output dims constant → region encoded at full res → crisp) and confirms when applied. Client renders a **residual** transform (residual zoom = frameViewport.w / target.w) → instant digital zoom during the pinch that settles to crisp host-cropped 1:1 when frame==target. Adversarial multi-agent review (12 agents) found+fixed 4 real issues: setViewport is `async -> Bool` awaited in-order in the inbound loop (no orphaned/racing confirm Tasks), confirms only on updateConfiguration success (no phantom-crop settle), dedup+1% near-full threshold (no reconfig thrashing). **Tests 72/25 green; host + iOS build clean.**
  ...
  - **ON-DEVICE #2 (user, 2026-06-03):** fill works great, but still blurry at moderate zoom → **zoom crispness fix** (commit `4591ef7`, build-green, device-verify pending). Root cause: the crop request was square (display-aspect) → for a wide display it clamped to the whole display until extreme zoom → host sent full display, client digitally magnified. Fix: client cropRequest = **tight visible-window region** (not squared) + **hysteresis** (keep crop while panning inside it; re-crop on edge/zoom); host **sizes its output buffer to the crop's native pixels** (visible region at full display density → crisp), width snapped to 16px, **height derived from width preserving exact crop aspect** (no stretch; matches client). Adversarially reviewed (8 agents) → fixed independent-snap aspect distortion + pan-reconfig-every-tick. Remaining (roadmap "zoom follow-ups"): adaptive bitrate, jerky tuning (incl. smoothing the brief rebuild on zoom steps). Extreme-zoom pixelation now inherent (native screen density).
