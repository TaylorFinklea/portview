...
- NEXT (fleet): remaining ready incl. zja (transport error surface), msy (tag registry CaseIterable), usy (starvation test), bnz (actors), 950 (CI gate), then the tag-adding media/protocol work (nl1 m-pingpong etc. — MUST update GoldenFrameTests+EnumTests when adding tags 27/28), cc-target (iOS, needs xcodebuild), and lead-tier specs (t-spec, r-cloudkit-spec, QUIC lane spec). NOT pushed.
