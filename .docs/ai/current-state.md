...
- **Fleet WAVE 1 DONE (8 beads, Sonnet in-session, verified green, committed 027c93b→bb090ed, NOT pushed)**: w-skip (unknown-tag skip — wire-safety root), w-golden (KAT+tag registry), m-bitrate (live setAverageBitRate), t-filecap (file caps), x-capturerace (CaptureEngine configLock), t-lockgate (host-side lock gate; paused is static-backed to cover all per-connection injectors), x-deadline (first-message deadline), cc-hostsplit (HostControl/DisplayRegistry own files). Full `swift test` = **219** green (was 183). Beads closed.
- **Fleet WAVE 2 DONE (4 beads, verified, committed e871a8b/b8de3e9/d26e3b8/fab94cc + app fix, NOT pushed)**: msy (tag-range doc + CaseIterable + EnumTests), zja (transport surfaces decode errors via processIncoming instead of silent stall), usy (serve-cap starvation regression test), bnz (SASPairingControl + DisplayRegistry → actors). LEAD FIXUPS during verify: zja was left uncommitted by its agent (finished+committed by me); bnz broke the macOS app target (HostAppModel called now-async openWindow/closeWindow synchronously) — fixed with Task{await} + confirmed `xcodebuild PortviewHost` builds. `swift test` 222 green; wire-safety epic (screenshare-1n6) now fully implemented (device-verify still pending on real hardware).
- NEXT (fleet): tag-adding media/protocol work (nl1 m-pingpong + m-feedback — MUST also add tags 27/28/29 to GoldenFrameTests + EnumTests or those gates fail, by design), cc-target (iOS PortviewClientCore — needs xcodebuild, run solo), 950 (CI Makefile), and lead-tier specs (t-spec, r-cloudkit-spec, QUIC lane) which need lead ownership not headless loop. NOT pushed. Device-verify queue unchanged.
