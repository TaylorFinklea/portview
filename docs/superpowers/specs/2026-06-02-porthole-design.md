...
> **Porthole** is an open-source, native-feeling iPhone → Mac screen-sharing and control app. It replaces the laggy VNC experience with a hardware-codec video stream over QUIC, the way Apple's own Mac-to-Mac screen sharing feels — but fully open, end to end.
...
| D7 | Name / license | **Porthole** / **Apache-2.0** | — |
...
- **iPhone-as-source** (sharing the phone screen to the Mac). Porthole is iPhone-as-controller only.
...
│   Discovery        Bonjour advertise  _porthole._udp                   │
...
The protocol is the riskiest, most-shared piece and is built/tested first. It is implemented in a pure-Swift `PortholeProtocol` package with deterministic binary encoding and round-trip tests — no Xcode, no device, `swift test`-able.
...
- `PortholeProtocol` — lane definitions, message types, binary framing, handshake state machine, version negotiation. Pure Swift, fully unit-tested.
- `PortholeTransport` — thin QUIC wrapper over Network.framework (listener + connection + per-lane streams/datagrams). Shared by both apps.

**Mac host (`PortholeHost`, menu-bar app):** `CaptureEngine`, `VideoEncoder`, `AudioTap`, `InputInjector`, `QuicServer`, `Discovery`, `PairingStore`, `PermissionsCoordinator`, `AppLifecycle` (SMAppService).

**iPhone client (`PortholeClient`, SwiftUI):** `Discovery`, `QuicClient`, `VideoDecoder`, `MetalRenderer`, `InputController`, `SessionView`, feature modules (clipboard/files/audio/displays), `PairingFlow` (QR scan), `SavedMacsStore`.
...
- **Unit (TDD):** `PortholeProtocol` message encode/decode round-trips, version negotiation, framing edge cases; input coordinate mapping (multi-monitor). Swift Testing.
...
- **M0 — Walking skeleton:** `PortholeProtocol` package (TDD) + minimal `PortholeTransport`; host captures one display → HEVC encode → QUIC → client decode → Metal render. No input/pairing, hardcoded LAN address. **Verify:** live Mac screen on the phone, < 50 ms.
...
porthole/
  ...
  Sources/
    PortholeProtocol/                # pure-Swift wire protocol  ← M0 starts here
    PortholeTransport/               # QUIC wrapper (Network.framework)
  Tests/
    PortholeProtocolTests/
    PortholeTransportTests/
  apps/
    PortholeHost/                    # macOS menu-bar app (Xcode project)
    PortholeClient/                  # iOS app (Xcode project)
