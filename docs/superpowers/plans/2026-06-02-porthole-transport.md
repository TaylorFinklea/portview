# PortholeTransport Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans. Steps use checkbox (`- [ ]`) syntax.

**Goal:** Build `PortholeTransport`, the QUIC transport that carries `PortholeProtocol` messages between the Mac host and iPhone client over a single encrypted connection with multiple logical lanes.

**Architecture:** A SwiftPM library depending on `PortholeProtocol`, importing Apple's `Network` framework. It wraps QUIC behind small Porthole-owned types so the wire mechanics can evolve without touching the apps. Reliable QUIC streams carry the control/input/clipboard/files lanes; the **video lane uses one short-lived unidirectional QUIC stream per frame** (no cross-frame head-of-line blocking; stale frames abandoned by stream reset). The QUIC datagram flow is available (confirmed in SDK) and reserved as a future sub-frame latency optimization.

**Tech Stack:** Swift 6.2, `Network.framework` (QUIC), `Security`/`CryptoKit` for the self-signed TLS identity, Swift Testing. Depends on `PortholeProtocol`.

**Scope (M0):** establish a QUIC connection host↔client on localhost, run the `PortholeProtocol` handshake over it, and stream `VideoFrame` messages host→client. Bonjour discovery and real device pairing are **out of scope here** (M1 — the M0 client hardcodes the host address). The TLS identity in M0 is a self-signed cert generated on the host; client trust pinning is stubbed to "accept the host's presented cert and record its fingerprint" (real pin-from-QR lands in M1).
...
- **This plan targets the classic `NWConnection`/`NWListener` + `NWProtocolQUIC.Options` API.** Rationale: it is stable since macOS 12 / iOS 15, fully documented, and its multiplexed-stream + datagram + TLS-identity surface is well understood. Because all QUIC usage is hidden behind Porthole-owned types (below), migrating to the new `NetworkConnection` API later is a contained, app-invisible refactor. (A spike to evaluate the new API is a fine future task, not a v1 blocker.)
...
**Files:**
- Modify: `Package.swift` (add `PortholeTransport` target + test target, both depending on `PortholeProtocol`)
- Create: `Sources/PortholeTransport/TLSIdentity.swift` (self-signed identity for the listener)
- Create: `Sources/PortholeTransport/QUICParameters.swift` (builds `NWParameters` for QUIC with our ALPN + limits)
- Create: `Sources/PortholeTransport/PortholeTransport.swift` (namespace + ALPN constant)
- Test: `Tests/PortholeTransportTests/LoopbackSpikeTests.swift`

- [ ] **Step 1: Extend `Package.swift`**

Add to `products` and `targets` (keep the existing `PortholeProtocol` entries):
...
```swift
// Sources/PortholeTransport/PortholeTransport.swift
...
```swift
// Sources/PortholeTransport/TLSIdentity.swift
import Foundation
import Security
...
```swift
// Sources/PortholeTransport/QUICParameters.swift
import Foundation
import Network

/// Builds `NWParameters` for a Porthole QUIC endpoint.
...
```swift
// Tests/PortholeTransportTests/LoopbackSpikeTests.swift
import Testing
import Foundation
...
```bash
git add Package.swift Sources/PortholeTransport Tests/PortholeTransportTests
...
**Files:**
- Create: `Sources/PortholeTransport/MessageChannel.swift`
- Test: `Tests/PortholeTransportTests/MessageChannelTests.swift`

- [ ] **Step 1: Write the failing tests**

```swift
// Tests/PortholeTransportTests/MessageChannelTests.swift
import Testing
@testable import PortholeTransport
@testable import PortholeProtocol
...
```swift
// Sources/PortholeTransport/MessageChannel.swift
...
```bash
git add Sources/PortholeTransport/MessageChannel.swift Tests/PortholeTransportTests/MessageChannelTests.swift
git commit -m "feat(transport): add MessageChannel (framing over a lane)"
```

---

### Task 3: PortholeConnection — lanes over QUIC + handshake-over-QUIC test
...
**Files:**
- Create: `Sources/PortholeTransport/PortholeConnection.swift`
- Create: `Sources/PortholeTransport/InboundMessage.swift`
- Test: `Tests/PortholeTransportTests/HandshakeOverQUICTests.swift`

- [ ] **Step 1: Define the inbound event type**

```swift
// Sources/PortholeTransport/InboundMessage.swift
import PortholeProtocol
...
```swift
// Tests/PortholeTransportTests/HandshakeOverQUICTests.swift
import Testing
import Foundation
@testable import PortholeTransport
@testable import PortholeProtocol
...
@Suite struct HandshakeOverQUICTests {
    /// Full PortholeProtocol handshake driven over a real localhost QUIC connection,
    /// then one VideoFrame delivered host->client on the video lane.
    @Test func handshakeAndFirstVideoFrameOverQUIC() async throws {
        // Implementer: stand up host + client PortholeConnection on 127.0.0.1.
...
- [ ] **Step 3: Implement `PortholeConnection`**

Design (implement against verified APIs; signatures below are the *intended surface*, not prescribed Network calls):

```swift
// Sources/PortholeTransport/PortholeConnection.swift
import Foundation
import Network
import PortholeProtocol

/// One live QUIC connection exposing Porthole's logical lanes.
public final class PortholeConnection: @unchecked Sendable {
...
Run: `swift test`
Expected: PASS (PortholeProtocol + PortholeTransport).
...
```bash
git add Sources/PortholeTransport Tests/PortholeTransportTests .docs/ai/current-state.md .docs/ai/roadmap.md
git commit -m "feat(transport): PortholeConnection lanes over QUIC; handshake+video e2e"
```
...
**Type consistency:** `MessageChannel.outbound/inbound`, `InboundMessage(lane:message:)`, `PortholeConnection.send(_:on:)`/`sendVideoFrame(_:)`/`inbound`/`close()`, `QUICParameters.server(identity:)`/`client(pinnedFingerprint:)`, `TLSIdentity.makeSelfSigned(commonName:)`, and `PortholeTransport.alpn` are referenced consistently. Reuses `PortholeProtocol` types (`Frame`, `FrameDecoder`, `AnyMessage`, `Lane`, `VideoFrame`, handshake messages) verbatim.

**Honest risk note:** Task 1 is deliberately a spike because the QUIC `NWMultiplexGroup` stream model and `SecIdentity` creation are the highest-uncertainty pieces. If the classic API proves awkward for per-frame unidirectional streams, evaluate the new `NetworkConnection`/`NetworkListener` + `QUIC` builder API (present in this SDK) — the Porthole-owned types insulate the apps either way.
