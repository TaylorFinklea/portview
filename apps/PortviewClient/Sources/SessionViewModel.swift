...
final class SessionViewModel: ObservableObject {
    ...
    nonisolated static func windowCovered(_ window: CGRect, by f: CGRect,
                                          marginFraction: CGFloat = 0.12, looseFactor: CGFloat = 4.2) -> Bool {
        // `looseFactor` must exceed the fresh crop's window-multiple so a just-applied crop reads as
        // covered (else it re-crops every frame): padding 0.9 → 2.8×window, snapped up by the ≤1.25×
        // capture-size ladder → ≤3.5×; 4.2 clears it.
