...
/// guard that a nudge (`wantsReconnect = 1`) always bypasses; a per-host wall-clock rate limit; and
/// the "one host's epoch must never suppress another host's" isolation the per-host map exists for.
@Suite struct ReWakeDecisionTests {
    ...
    @Test func unknownHostIsIgnored() {
        ...
        let action = ReWakeDecision.evaluate(beacon: beacon, savedHosts: [], lastHandledEpochs: [:], now: now)
    ...
    @Test func routineBeaconWithEpochAtOrBelowLastHandledIsIgnored() {
        ...
        // Equal epoch (exact replay) and a strictly older epoch both must be ignored.
        for epoch in [lastHandled, lastHandled - 1] {
            ...
            let action = ReWakeDecision.evaluate(
                beacon: beacon, savedHosts: saved, lastHandledEpochs: [pinA: lastHandled], now: now
            )
            #expect(action == .ignore)
        }
    }
    ...
    @Test func routineBeaconWithFreshEpochIsActedOn() {
        let now = Date()
        let saved = [ReWakeDecision.SavedHost(pinHex: pinA, host: "10.0.0.5")]
        // Previous handling was long enough ago to clear the rate limit too.
        let lastHandled = epochMicros(now.addingTimeInterval(-ReWakeDecision.minActInterval - 10))
    ...
    // MARK: required test (b) — one host's epoch never suppresses another
    ...
    @Test func oneHostsLargeEpochNeverSuppressesADifferentHost() {
        ...
        // Host A has a huge last-handled epoch (long-running desktop); host B is fresh and its own
        // epoch numerically reads well below host A's — must not matter, the map is keyed per host.
        ...
        let action = ReWakeDecision.evaluate(
            beacon: beaconForB, savedHosts: saved, lastHandledEpochs: [pinA: hostALastHandled], now: now
        )

        #expect(action == .reachabilityProbe(endpoint: .init(host: "10.0.0.9", port: 4433), pin: pinB))
    }

    // MARK: required test — per-host rate limit

    @Test func rateLimitIgnoresAnotherActionTooSoonAfterTheLast() {
        let now = Date()
        let saved = [ReWakeDecision.SavedHost(pinHex: pinA, host: "10.0.0.5")]
        let lastHandled = epochMicros(now.addingTimeInterval(-ReWakeDecision.minActInterval / 2))
        let beacon = HostBeaconRecord(recordName: pinA, hostName: "Mac", port: 4433, epoch: epochMicros(now), wantsReconnect: 1)
    ...
    @Test func actsAgainOnceTheRateLimitWindowHasElapsed() {
        let now = Date()
        let saved = [ReWakeDecision.SavedHost(pinHex: pinA, host: "10.0.0.5")]
        let lastHandled = epochMicros(now.addingTimeInterval(-ReWakeDecision.minActInterval - 1))
        let beacon = HostBeaconRecord(recordName: pinA, hostName: "Mac", port: 4433, epoch: epochMicros(now), wantsReconnect: 1)
    ...
    @Test func firstEverBeaconForAKnownHostHasNothingToRateLimitAgainst() {
        ...
        let action = ReWakeDecision.evaluate(beacon: beacon, savedHosts: saved, lastHandledEpochs: [:], now: now)
